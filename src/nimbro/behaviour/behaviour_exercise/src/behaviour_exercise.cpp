...
// Constructor
RobotSC::RobotSC() : StateController("RobotSC")
	, m_be_run("/behaviour_exercise/run", false)
	, m_be_use_obstacle("/behaviour_exercise/use_obstacle", false)
	, m_be_place_robot("/behaviour_exercise/on_run/place_robot", true)
	, m_be_rand_robot("/behaviour_exercise/on_run/random_robot", false)
	, m_be_rand_ball("/behaviour_exercise/on_run/random_ball", false)
	, m_be_rand_goal("/behaviour_exercise/on_run/random_goal", false)
	, m_be_rand_obst("/behaviour_exercise/on_run/random_obstacle", false)
	, m_be_imperfect_meas("/behaviour_exercise/use_imperfect_measurements", false)
	...
	, m_usingPositiveGoal(true)
{
	...
	m_srv_setOdom = nh.serviceClient<gait::SetOdom>("/gait/setOdom");
	...
	if(!setParam("/gait/odometry/use_simulation_odometry", "1")) ROS_WARN("Failed to set config server parameter '/gait/odometry/use_simulation_odometry'!");
...
bool RobotSC::setOdom(double x, double y)
{
	...
	{
		gait::SetOdom pos;
		pos.request.fieldX = x;
		pos.request.fieldY = y;
...
// Gait functions
void RobotSC::sendGaitCommand(gait::GaitCommand cmd)
{
	// Constants
