...
void WalkControl::initPlugin(qt_gui_cpp::PluginContext& context)
{
	...
	m_pub_cmd = getNodeHandle().advertise<gait::GaitCommand>("/gaitCmd", 1);

	m_sub_perf = getNodeHandle().subscribe("/gait/performance/", 10, &WalkControl::perf_changed, this);
	qRegisterMetaType<gait::GaitPerformanceConstPtr>("gait::GaitPerformanceConstPtr");

	connect(this, SIGNAL(perf_changed(gait::GaitPerformanceConstPtr)), this, SLOT(handlePerf(gait::GaitPerformanceConstPtr)), Qt::QueuedConnection);
...
void WalkControl::handlePerf(const gait::GaitPerformanceConstPtr& data)
{
