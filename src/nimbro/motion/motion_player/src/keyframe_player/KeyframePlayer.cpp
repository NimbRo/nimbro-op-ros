...
{
	return addKeyframe(Keyframe(t, x, v));
}

bool KeyframePlayer::addKeyframe(double t, double x, double v, double eff)
{
	return addKeyframe(Keyframe(t, x, v, eff));
}
...
Keyframe KeyframePlayer::evaluateAt(double t)
{
	...
	state.t = t;
	state.eff = keyframes[effortIndex].eff;
...
void KeyframePlayer::calculateCommands()
{
	...
	for (int i = 0; i < keyframes.length()-1; i++)
	{
		...
		kf1 = keyframes[i+1];
		kf = kf0;
		kf.eff = kf1.eff; //Hack. Testing  a effort bug.
