...
void ConfigClient::init()
{
	...
	{
		newDescription.name = "/gait/" + reformat(parameterList[i]).toStdString();
		newDescription.default_value = "0";
		newDescription.step = gait::config.sliderFactors[parameterList[i]];
		newDescription.min = -100 * gait::config.sliderFactors[parameterList[i]];
		newDescription.max = 100 * gait::config.sliderFactors[parameterList[i]];
